...
<head>
  ...
  <title>{{ $post->title }} - PostBro</title>
...
<div class="container mx-auto p-6">
  <!-- Hero Section -->
  <div class="relative h-[400px] rounded overflow-hidden shadow-lg">
    <img src="{{ asset('storage/' . $post->image) }}" alt="Post Image" class="w-full h-full object-cover brightness-75"/>
    <!-- Overlay -->
    <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
      <h1 class="text-5xl font-extrabold text-white drop-shadow-lg">{{ $post->title }}</h1>
      <p class="text-lg text-white mt-2">{{ $post->description }}</p>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 mt-10">
    ...
    <div class="lg:col-span-1 bg-gray-800 rounded shadow-xl p-6">
      <h2 class="text-2xl font-bold text-white mb-4">Post Details</h2>
      <ul class="space-y-4">
        <li class="flex justify-between text-white font-semibold border-b pb-2">
          <span>Price:</span>
          <span class="text-green-600">{{ $post->price ?? '0.00 '}} € </span>
        </li>
        <li class="flex justify-between text-white font-semibold border-b pb-2">
          <span>Date:</span>
      ...
      <!-- Call-to-Action Button -->
        <button class="w-full mt-6 bg-[#fb8c00] text-white py-3 rounded-lg text-lg font-bold hover:bg-[#fb8c00] transition duration-300 shadow-lg">Download Your Ticket</button>
    </div>
    
    <!-- Main Content (Description & Organizer) -->
    <div class="lg:col-span-2 space-y-8">
      <!-- Post Description -->
      <div class="bg-gray-800 rounded shadow-xl p-6">
      ...
      <div class="bg-gray-800 rounded shadow-xl p-6 flex items-center">
        <img 
          src="" 
          alt="Organizer Avatar" 
          class="w-16 h-16 rounded-full shadow-md"/>
        <div class="ml-4">
          <p class="text-xl font-semibold text-gray-900">{{ $post->user->name }} <i class="fa-solid fa-circle-check" style="color: #63E6BE;"></i>  </p>
          <p class="text-white">Email : {{ $post->user->email }} </p>
        </div>
      </div>
      
    </div>

    <div class="flex justify-center col-span-3 w-full items-center">
      <div class="h-80 px-7 w-full rounded bg-gray-800 p-4 shadow-md border">
        ...
        <textarea name="contenu" class="h-40 px-3 text-sm py-1 mt-5 outline-none border-gray-300 w-full resize-none border rounded-lg placeholder:text-sm" placeholder="Add your comments here"></textarea>  
        
        <div class="flex justify-between mt-2"> 
            <p class="text-sm text-blue-900 ">Enter atleast 15 characters</p>
            <button type="submit" name="submit"class="h-12 w-[150px] bg-[#fb8c00] text-sm text-white rounded-lg transition-all cursor-pointer hover:bg-blue-600">
                Submit comment
            </button>
        </div>  
        </form>
...
<!-- Footer -->
  <footer class="mt-8 bg-gray-900 dark:bg-gray-900 pt-9">
			...
			<div class="flex flex-col justify-between sm:px-[18px] md:flex-row md:px-10">
				<div class="md:w-[316px]">
					...
					<h1 class="text-white font-extrabold">
						<span class="text-orange-500">PostBRO
					</h1>
					</p>
					</p>
				...
				<div class="mt-6 flex w-full flex-col justify-between text-white sm:flex-row md:mt-0 md:max-w-[341px]">
					<div class="">
						...
						<ul>
							...
							<li class="mt-[15px]">
								<a class="text-deutziawhite hover:text-deutziawhite/80 font-inter text-[15px] font-normal hover:font-semibold" href="/terms-and-conditions">Terms and conditions</a>
							</li>
							<li class="mt-[15px]">
								<a class="text-deutziawhite hover:text-deutziawhite/80 font-inter text-[15px] font-normal hover:font-semibold" href="/privacy-policy">Privcay policy</a>
							</li>
						</ul>
					</div>
