                ...
                <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-6 space-y-6">
                    ...
                    <div class="container mx-auto mt-8 px-4">
                        ...
                        <div class="bg-white shadow-md rounded-lg p-6">
                            <form action="{{ route('posts.update',$post->id) }}" method="POST" onsubmit="alert('Form submitted!');" enctype="multipart/form-data" id="PostForm" class="">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="description" id="description" rows="4" value="{{ $post->description }}"
                                        class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $post->description }}</textarea>
                                    @error('description')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="categorie_id" id="categorie_id"
                                        ...
                                        <option value="{{ $post->categorie_id }}"
                                            {{ old('categorie_id') == $post->categorie_id ? 'selected' : '' }}>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="status" id="status" class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        <option value="{{$post->status}}" {{ $post->status ? 'selected' : '' }}>{{$post->status}}</option>
                                        <option value="actif" {{ old('status') == 'actif' ? 'selected' : '' }}>Active</option>
                                        <option value="brouillon" {{ old('status') == 'brouillon' ? 'selected' : '' }}>Draft</option>
                                        <option value="archivé" {{ old('status') == 'archivé' ? 'selected' : '' }}>Archived</option>
                                    </select>
                                    @error('status')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                    @enderror
